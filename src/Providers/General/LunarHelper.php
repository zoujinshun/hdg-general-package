 ...
 * 农历、节日、节气
 * Class Lunar
 * @package Vaedly\HdgGeneralPackage\Providers\Common
 */
class LunarHelper
{
    var $MIN_YEAR = 1891;
     ...
     * @return bool
     */
    public function isWinterSolstice($date)
    {
        ...
        $_l = intval($year / 4);
        $_c = 21.94;
        $day = intval($year * $_d + $_c - $_l);
        if (in_array($date, [2021])) {
            $day--;
        }
        return $d == $day;
    }
     ...
     * @return bool
     */
    public function isQingMing($date)
    {
        list($y, , $d) = explode('-', $date);
        $year = substr($y, -2) + 0;
        $_d = 0.2422;
        $_l = intval($year / 4);
        $_c = 4.81;
        if ($this->getCentury($y) == 20) {
            $_c = 5.59;
        }
        $day = intval($year * $_d + $_c - $_l);
        return $d == $day;
     ...
     * 是否除夕
     */
    public function isChineseNewYearEve($date)
    {
        $date = date('Y-m-d', strtotime("$date + 1 day"));
        [$year, $month, $day] = explode('-', $date);
        $solar = $this->convertSolarToLunar($year, $month, $day);
        if(is_array($solar) && $solar[1] == '正月' && $solar[2] == '初一'){
            return true;
        }
        return false;
    }
    ...
    function getFestival($today, $nl_info = false, $config = 1)
    {
        if ($config == 1) {
            ...
            $arr_solar = array('01-01' => '元旦', '02-14' => '情人节', '03-08' => '妇女节', /*'03-12' => '植树节',*/ '04-01' => '愚人节', '05-01' => '劳动节', '05-04' => '青年节', '06-01' => '儿童节', '07-01' => '建党节', '08-01' => '建军节', '09-10' => '教师节', '10-01' => '国庆节', '10-31' => '万圣节', '11-25' => '感恩节', '12-24' => '平安夜', '12-25' => '圣诞节');
        ...
        if ($m == 12 && in_array($d, [21, 22, 23])) {
            if ($this->isWinterSolstice($today)) {
                array_push($festivals, "冬至");
            }
        }

        if(in_array($m, [12, 1, 2])){
            if($this->isChineseNewYearEve($today)){
                array_push($festivals, "除夕");
            }
        }

        if ($m == 4 && in_array($d, [4, 5, 6])) {
            if ($this->isQingMing($today)) {
                array_push($festivals, "清明节");
            }
        }
